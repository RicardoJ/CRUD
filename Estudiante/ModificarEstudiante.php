...
<form name="form1" method="post"  action="../Estudiante/ModificarEstudiante.php" onsubmit ="return validar();">
...
if(isset($_POST['modificar'])){
    ...
    if(mysql_num_rows($busqueda) > 0){
        $resultado= mysql_query("UPDATE `estudiante` SET `NombreE`='$nombre',`ApellidoE`='$apellido',`GeneroE`='$genero' WHERE `ID_E` ='$id'",$con)
        or die("Error al actualizar");
 $mensaje ="los datos se han actualizado";
 print "<script>alert('$mensaje'); window.location= '../Estudiante/ModificarEstudiante.php';</script>" ;
}else{
